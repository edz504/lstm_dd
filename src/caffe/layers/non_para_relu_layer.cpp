...
void NonParaReLULayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* beta_data = this->blobs_[0]->cpu_data();
  ...
  for (int i = 0; i < count; ++i) {
    ...
    top_data[i] = std::max(bottom_data[i], Dtype(0))
        + beta_data[c] * sigmoid(bottom_data[i]);
  }
}
...
void NonParaReLULayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    caffe_set(this->blobs_[0]->count(), Dtype(0), beta_diff);
    for (int i = 0; i < count; ++i) {
      int c = (i / dim) % channels / div_factor;
      beta_diff[c] += top_diff[i] * sigmoid(bottom_data[i]);
    }
  }
  // Propagate to bottom
  if (propagate_down[0]) {
