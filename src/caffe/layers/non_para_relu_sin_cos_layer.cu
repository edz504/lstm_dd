...
void NonParaReLUSinCosLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* beta_cos_data = this->blobs_[0]->gpu_data();
...
void NonParaReLUSinCosLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    Dtype* beta_cos_diff = this->blobs_[0]->mutable_gpu_diff();
    ...
    caffe_gpu_set<Dtype>(this->blobs_[0]->count(), Dtype(0), beta_cos_diff);
    ...
    if (channel_shared_) {
      ...
      caffe_gpu_set(this->blobs_[0]->count(), Dtype(dsum_cos), beta_cos_diff);
    }
  }
