...
void EuclideanLossDrivingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  cudaMemcpy(label,label_cuda,sizeof(Dtype)*num*output_dim,cudaMemcpyDeviceToHost);
...
template <typename Dtype>
void EuclideanLossDrivingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(EuclideanLossDrivingLayer);
