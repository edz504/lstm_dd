...
/*
/////////////////// commented by chenyi, for loading .solverstate files
  if (resume_file) {
...
/////////////////// by chenyi, for loading both .solverstate and .caffemodel files
  if (resume_file) {
    string resume=resume_file;
    int n=resume.length();
    if (resume.substr(n-3,3)=="txt") {
       iter_ = 0;
       LOG(INFO) << "Restoring previous caffemodel from " << resume_file;
       NetParameter trained_net_param;
       ReadProtoFromTextFile(resume_file, &trained_net_param);
       net_->CopyTrainedLayersFrom(trained_net_param);
    }else if (resume.substr(n-10,10)=="caffemodel") {
       iter_ = 0;
       LOG(INFO) << "Restoring previous caffemodel from " << resume_file;
       NetParameter trained_net_param;
       ...
       LOG(INFO) << "Restoring previous solverstatus from " << resume_file;
       Restore(resume_file);
    }
  }
/////////////////// by chenyi, for loading both .solverstate and .caffemodel files
