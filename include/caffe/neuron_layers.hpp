...
////////////////////////////////////////////////////////// by chenyi
template <typename Dtype>
class SigmoidHistLayer : public NeuronLayer<Dtype> {
 public:
  explicit SigmoidHistLayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param) {}

  virtual inline const char* type() const { return "SigmoidHist"; }
...
#ifdef USE_CUDNN
template <typename Dtype>
class CuDNNSigmoidHistLayer : public SigmoidHistLayer<Dtype> {
 public:
  explicit CuDNNSigmoidHistLayer(const LayerParameter& param)
      : SigmoidHistLayer<Dtype>(param), handles_setup_(false) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual ~CuDNNSigmoidHistLayer();
