...
//////////////////////////////////////////////////// by chenyi
template <typename Dtype>
class EuclideanLossDrivingLayer : public LossLayer<Dtype> {
 public:
  explicit EuclideanLossDrivingLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "EuclideanLossDriving"; }
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
  }
...
template <typename Dtype>
class EuclideanLossDrivingDMLayer : public LossLayer<Dtype> {
 public:
  explicit EuclideanLossDrivingDMLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "EuclideanLossDrivingDM"; }
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
  }
